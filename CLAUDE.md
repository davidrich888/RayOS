...
- **三態機制**: 前端支援 null/true/false 三態。Notion checkbox 只有 true/false，所以 `true`(✓) 存 checkbox；`false`(✗) 額外記進 `FailedHabits` rich_text 欄，read 時由 `triState()` 還原（讓 ✗ 跨裝置存活）；`null`(空白)＝checkbox unchecked 且不在 FailedHabits。write/read 邏輯見 `js/notion-sync.js`（`failedHabitsProp` / `parseFailedSet` / `triState`）。⚠️ n8n fallback 寫入路徑目前不更新 FailedHabits（僅 Direct 路徑支援跨裝置 ✗）
...
- **Fields**: Title(title), Target(number=單次出金金額), Current(number=已完成里程碑數), Exams(number=考試階段數), Milestones(rich_text=JSON), Order(number), Status(select: active/completed), CreatedDate(date), CompletedDate(date)
- **同步方式**: Notion Direct（透過 Vercel `/api/notion` proxy），跟 Plan 相同模式
- **ID 格式**: `goal_{fullNotionUUID}`
- **資料流**: 前端為主要編輯器 → localStorage 快取 → Notion 當資料庫
- **首次載入**: 頁面載入時自動 render + sync（目標是預設 tab）
- **里程碑結構**: 支援自訂區段（sections）。預設=N考試+12出金；自訂=任意區段組合（如考試×10+出金×5+copytrade×4+出金5000×5）
- **Milestones 格式**: 舊=`[false,true,...]`（boolean array）；新=`{sections:[{label,count,amountEach,startNum},...], values:[...]}`
- **拖曳排序**: SortableJS，拖曳把手 ⠿，排序同步 Order 欄位到 Notion
- **表單欄位**: 目標名稱 + 單次出金金額 + 考試數（預設 2）

### 🎯 System Tracker
- **Database ID**: `578f08b33c7a48efb4b681329762f6c8`
- **Data Source (Collection)**: `c02bb48c-ca37-4ed7-8013-6f2301268814`
- **位置**: 📊 RayOS Data → 🤖 AI 資料庫 → 🎯 System Tracker
- **前端**: AI Agents 頁面 → Status Board tab（`js/status-board.js`）
- **同步方式**: Notion Direct（透過 Vercel `/api/notion` proxy），只讀
- **更新方式**: **只由 Claude Code 維護** — 執行 `/status-sync` 全量同步或個別更新
- **用途**: 統整所有 N8N workflows、專案、功能、Auto Tasks、Skill/Command 的狀態追蹤
- **Fields**: Title(title), Category(select), Status(select), Priority(select), Area(select), Progress(number), NextStep(rich_text), Blocker(rich_text), LastUpdated(date), RepoPath(rich_text), Notes(rich_text)
- **Status 值**: Done / WIP / Blocked / Backlog / Active / Archived
- **Category 值**: N8N Workflow / Project / Feature / Automation / Auto Task / Skill/Command
...
- **功能**: 最多 3 個同時進行的衝刺，30 格 grid 每日勾選，SortableJS 拖曳排序，刪除按鈕（本地+Notion 歸檔）
- **三態格子**: 空白 → ✓（完成，金色）→ ✗（失敗，紅色，可填原因）→ 空白。hover 顯示失敗原因 tooltip
- **資料流**: 前端為主要編輯器 → localStorage 快取 → Notion 當資料庫
- **Days 欄位格式**: JSON 字串，值為 `true`（✓）、`{"fail":true,"reason":"..."}`（✗）、或不存在（空白）
- **歷史衝刺**: 不顯示（已移除），已放棄/完成的衝刺只存在 Notion
...
- archive/ — 舊版備份和棄用檔案
  ...
  - scripts/ — 舊版工具腳本
  - expense-bills/ — 信用卡帳單原始檔（CSV/XLSX/PDF）
- scripts/ — 自動化腳本
  - sync_taishin_expense.py — 台新帳單同步（Playwright + Vision OCR）
  - sync_cathay_expense.py — 國泰帳單同步（gws + pikepdf + pdfplumber）
  - reparse_expense.py — 重新分類所有歷史帳單（CSV/XLSX）
  - sync-trading-data.py — 交易數據同步
- data/ — 數據檔案
  - expense-transactions.json — 所有信用卡交易明細（台新+國泰合併）
