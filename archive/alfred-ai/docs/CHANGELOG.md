...
## v6.0 — 每日資訊整理（2026-02-16）

### 重大變更
- 完全移除 RSS 來源（品質不佳）
- 只保留 X/Twitter + YouTube Atom + IG 三個來源
- 每日精選從 10 則增加到 20 則
- X/Twitter maxTweets 從 10 增加到 50，取互動量前 40 則給 Claude
- 新增 X 推文依互動量排序（likes + retweets×2）
...
### Prompt 調整
- ⚡ 行動建議回歸：只在真正值得行動時才加，不是每則都要
- 每則標題前標示來源平台 [X]、[YT]、[IG]
- max_tokens 從 3000 增加到 4096（配合更多內容輸出）

### 技術調整
- Claude API timeout 從 60s 增加到 120s
- IG 抓取數量從 每帳號 3 則增加到 5 則
- Format Digest 統計移除 RSS 計數
- 版本號升級為 v6.0
